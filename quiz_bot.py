...
"""
SUPER QUIZ BOT v3.2
Asosiy menyu: Testlar bo'limi (Mikro | Makro | Pul va Bank) + Reyting + Statistika
"""
...
def main_kb(uid):
    ...
    return ReplyKeyboardMarkup([
        ...
        [KeyboardButton(TX[lang]["btn_mikro"]),
         KeyboardButton(TX[lang]["btn_makro"])],
...
async def handle_reply_btn(u: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Har bir kategoriya to'g'ridan tugma orqali ham ochiladi
    for cat_key, cat in CATEGORIES.items():
        if text == f"{cat['emoji']} {cat[lang]}":
            await show_topic_inline(uid, cat_key, cid, ctx); return
